...
;
;******* Opcode 0x26
;The ASL2 instruction.


opcode26:SUBSP   4,i         ;Create two local words by pushing the stack.
LWSF:   .EQUATE 0            ;Creating a local word for the A register.
LWA:    .EQUATE 2           ;Create a local word for the A register.


;************ First shift ************
         ASLA
         STWA    LWA,s       ;Storing the results in LWA.
         MOVFLGA             ;Loading the status flags into A.

;The 7th bit in the NZVC flags byte I'd the V status bit: So when running MOVFLGA to get the status bits into A, the 7th bit on the rightmost byte is the V status bit: So the mask is to keep just that bit, and ignore everything else
         ANDA    0x0002,i   ;bitmask to preserve only the V bit (AND 0000 0000 0000 0010)
         STWA    LWSF,s     ;store the resulting status flags in LWSF
         LDWA    LWA,s      ;reload the shifted result back into A

;************ second shift ************
         ASLA                ;Do shift again
         STWA    LWA,s    ;Store results into atemp
         MOVFLGA             ;Load flags into A

;After the second OR the with the previous v-bit
         ORA     LWSF,s


;The equates 'NZVC' and 'NZVCA' to denote the positions of the status flags and the A register, respectively, in the ABI.
NZVC:    .EQUATE 6           ;Reserve position 0 in the ABI: is reserved for LWSF
NZVCA:   .EQUATE 7           ;Reserve position 1 in the ABI: is reserved for LWA



         STBA    NZVC,s      ;store the status flags into the ABI at position 'NZVC' using 'STBA'
         LDWA    LWA,s       ;A is loaded from the temporary location 'LWA' into the accumulator using 'LDWA'
         STWA    NZVCA,s     ;The value of A is stored in the ABI at position 'NZVCA' using 'STWA'
         ADDSP   4,i         ;deallocate local variables from the stack
         RET
;
;******* Opcode 0x27
...
opcode28:   LDWA         0x0002,i   ;Assert d
            ...
            SUBSP       10, i        ;Push stack to create 4 local words
Lprod:      .EQUATE 0               ;prod is at 0, s
Lmpr:       .EQUATE 2               ;mpr is at 2,s
Lmcand:     .EQUATE 4               ;mcand is at 4,s
LflagsV:    .EQUATE 6               ;flags is at 6,s
Lflags:     .EQUATE 8
PCBFLAGS:   .EQUATE 12              ;Position 0 in PCB for V-flag status, after one word reserved for old PC value
PCBA:       .EQUATE 13              ;Position 1 in PCB, for Accumulator value
            LDWA        0x0000,i    ;set A to 0
            STWA        Lprod,s
            STWA        Lmpr,s
            STWA        Lmcand,s
            STWA        LflagsV,s
            ...
            STBA        PCBFLAGS,s
            LDWA        opAddr,n    ;
            STWA        Lmpr,s      ;Lmpr is set with system global mpr, via A
            LDWA        PCBA,s
            STWA        Lmcand,s
	    LDWA	Lmpr,s	    ; load
	    ...
	    NEGA
	    STWA	Lmcand,s		
While:      LDWA        Lmpr,s      ;Load A with local mpr
            CPWA        0x0000,i    ;compare with 0
            BREQ        endLoop     ;get out of loop
            ANDA        0x0001,i    ;AND A with 1
            CPWA        0x0000,i    ;compare with 0
            BREQ        Divide      ;GOTO Divide
            LDWA        Lmcand,s    ;Load Lmcand into A
            ADDA        Lprod,s     ;Add A to Lprod
            STWA        Lprod,s     ;Store result into Lprod
            MOVFLGA                 ;Load status flags into A
            STWA        Lflags,s    ;Store flags from latest ADDA to reflect NZC
            ANDA        0x0002,i    ;Bitmask for V bit
            ORA         LflagsV,s    ;Check whether Lflags v bit is already 1
            STWA        LflagsV,s    ;Store flags into Lflags
Divide:     LDWA        Lmpr,s      ;Load A from multiplier
            ASRA                    ;Shift right divides by 2 ( :
            STWA        Lmpr,s      ;Store result in Lmpr
            LDWA        Lmcand,s    ;Load A with Lmcand
            ASLA                    ;multiply mcand by 2
            STWA        Lmcand,s    ;store result in local mcand on stack
            MOVFLGA                 ;Load status flags into A
            ANDA        0x0002,i    ;Bitmask for V bit
            ORA         LflagsV,s    ;Check whether Lflags v bit is already 1
            STWA        LflagsV,s    ;Store result in Lflags
            BR          While          ;Restart loop
endLoop:    LDWA        LflagsV,s   ;Load local flags into A
            ORA         Lflags,s    ;ORA (Lflags) with PCBFLAGS
            STBA        PCBFLAGS,s  ;Store A into flags portion of ABI
            LDWA        Lprod,s     ;Load A with product result
            STWA        PCBA,s      ;Store A into A portion of ABI
