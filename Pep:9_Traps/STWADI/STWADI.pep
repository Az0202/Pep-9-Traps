...
;++++++ I GOT THE THIS IDEA FROM THE BOOK PAGE 504 +++++

ARegPos:  .EQUATE 3           ;The A register is located at position 1 in ABI
PCPos:    .EQUATE 7           ;The Program Counter (PC) is located at position 5 in ABI
IRRegPos: .EQUATE 11          ;The Instruction Register (IR) is located at position 9 in ABI
;loads the value 0x0002 into the instruction register to set it to indirect addressing mode.
opcode26: LDWA    0x0002,i    
;stores the new value of the instruction register at the position specified by IRRegPos.
          STBA    IRRegPos,s   
;loads the value of the program counter into register A.
          LDWA    PCPos,s    
;adds 2 to the value of register A to increment the program counter by 2.
          ADDA    2,i         
;stores the new value of the program counter back into memory at the position specified by PCPos.
          STWA    PCPos,s    ;Store new PC
;calls a subroutine called setAddr to get the memory location to write to.
          CALL    setAddr     
;loads the memory address into register X.
          LDWX    opAddr,n    
;oads the value of register A from memory at position ARegPos.
          LDWA    ARegPos,s  
;stores the value of register A into memory at the location specified by register X.
          STWA    0,x        
          RET
;
;******* Opcode 0x27
