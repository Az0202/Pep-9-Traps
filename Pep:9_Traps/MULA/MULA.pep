...
;
;******* Opcode 0x28
;The MULA instruction.
opcode28:   LDWA        0x0002,i    ;Assert d
            STWA        addrMask,d
            CALL        assertAd
            CALL        setAddr     ;Set address of trap operand
            SUBSP       10, i       ;Increase the stack pointer by 10 to create space for 4 local words
LP:         .EQUATE 0               ;prod is at 0, s
LocalMp:    .EQUATE 2               ;LocalMp is at 2,s
LocalMCD:   .EQUATE 4               ;LocalMCD is at 4,s
Vflags:     .EQUATE 6                ;Vflags is at 6,s
LocalF:     .EQUATE 8
PCBVFLAG:   .EQUATE 12              ;Memory address in Process Control Block (PCB) for V-flag status, after one word reserved for old Program Counter (PC) value
PCBACC:     .EQUATE 13              ;Memory address in PCB for Accumulator value.
            LDWA        0x0000,i    ;Load the value 0 into the Accumulator (register A).
            STWA        LP,s
            STWA        LocalMp,s
            STWA        LocalMCD,s
            STWA        Vflags,s
            ...
            STBA        PCBVFLAG,s
            LDWA        opAddr,n    
            STWA        LocalMp,s   ;LocalMp is set with system global mpr, via A
            LDWA        PCBACC,s
            STWA        LocalMCD,s
	LDWA	LocalMp,s	; load LocalMp
	CPWA	0x0000,i    ; compare
	BRGE	While	; GO TO While
	NEGA                    ; Negate the two numbers: i faced a problem when the second number is negative but now it works!
	STWA	LocalMp,s
            LDWA	LocalMCD,s
	NEGA
	STWA	LocalMCD,s		
While:      LDWA        LocalMp,s   ;Load the value in memory at the location pointed to by LocalMp,s into the accumulator LDWA LocalMp,s
            CPWA        0x0000,i    ;Compare the value in the accumulator with the value 0x0000 CPWA 0x0000,i
            BREQ        endLoop     ;Branch to endLoop if the values in the accumulator and 0x0000 are equal BREQ endLoop
            ANDA        0x0001,i    ;Perform a bitwise AND of the value in the accumulator with 0x0001, storing the result in the accumulator ANDA 0x0001,i
            CPWA        0x0000,i    ;Compare the value in the accumulator with the value 0x0000 CPWA 0x0000,i
            BREQ        Divide      ;Branch to Divide if the values in the accumulator and 0x0000 are equal BREQ Divide
            LDWA        LocalMCD,s  ;Load the value in memory at the location specified by the pointer LocalMCD,s into the accumulator
            ADDA        LP,s        ; Add the value in the accumulator to the value in memory at the location specified by the pointer LP,s, storing the result in the accumulator
            STWA        LP,s        ;Store the value in the accumulator in memory at the location specified by the pointer LP,s
            MOVFLGA                 ;Move the status flags into the accumulator
            STWA        LocalF,s ;Store the status flags in memory at the location specified by the pointer Local_flags,s
            ANDA        0x0002,i    ;Perform a bitwise AND of the value in the accumulator with the bitmask 0x0002, storing the result in the accumulator
            ORA         Vflags,s    ;Perform a bitwise OR of the value in memory at the location specified by the pointer Vflags,s ... with the value in the accumulator, storing the result in the accumulator
            STWA        Vflags,s    ;Store the value in the accumulator in memory at the location specified by the pointer V_flags,s
Divide:     LDWA        LocalMp,s      ;Load the value in memory at the location specified by the pointer LocalMp,s into the accumulator
            ASRA                     ;Shift the bits in the accumulator one place to the right,effectively dividing the value by 2
            STWA        LocalMp,s    ;Store the result in the accumulator in memory at the location specified by the pointer LocalMp,s
            LDWA        LocalMCD,s   ;Load the value in memory at the location specified by the pointer LocalMCD,s into the accumulator
            ASLA                    ;Shift the bits in the accumulator one place to the left, effectively multiplying the value by 2
            STWA        LocalMCD,s  ;Store the result in the accumulator in memory at the location specified by the pointer LocalMCD,s
            MOVFLGA                 ;Move the status flags into the accumulator A
            ANDA        0x0002,i    ;Perform a bitwise AND of the value in the accumulator with the bitmask 0x0002,:  storing the result in the accumulator
            ORA         Vflags,s    ;Perform a bitwise OR of the value in memory at the location specified by the pointer Vflags,s. with the value in the accumulator, storing the result in the accumulator
            STWA        Vflags,s    ;Store the value in the accumulator in memory at the location specified by the pointer Vflags,s
            BR          While        ;Branch to the While label, restarting the loop
endLoop:    LDWA        Vflags,s   ;Load the value in memory at the location specified by the pointer V_flags,s into the accumulator
            ORA         LocalF,s    ;ORA Local_flags with PCB_VFLAG
            STBA        PCBVFLAG,s  ;Store A into flags portion of ABI
            LDWA        LP,s     ;Load A with product result
            STWA        PCBACC,s      ;Store A into A portion of ABI
            ADDSP       10,i         ;Pop stack to program-ready position
            RET
;
;******* Opcode 0x30
